...
def get_answer_using_template_with_history_backup(query, vectorstore, chat_memory):  
    ...
    for i, rel_doc in enumerate(relevant_documents):
        ...
        relevant_docs_context = f"{relevant_docs_context}{body}\n"  # append relevant_documents 
...
def get_answer_using_template(query, vectorstore, rag_type):        
    ...
    if word_kor:
        prompt_template = """\n\nHuman: 아래 문맥(context)을 참조했음에도 답을 알 수 없다면, 솔직히 모른다고 말합니다.Assistant의 친근한 대화하고, 상황에 맞는 구체적인 세부 정보를 충분히 제공합니다. 
